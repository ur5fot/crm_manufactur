...
crm_manufactur/
├── data/
│   ├── employees.csv              # Основные данные сотрудников (40 полей) - в gitignore
│   ├── fields_schema.csv          # Мета-схема: типы полей, метки, опции, UI конфигурация - в gitignore
│   ├── fields_schema.template.csv # Шаблон схемы для новых установок (tracked in git)
...
При первом развертывании создайте локальный файл схемы из шаблона:

```bash
cp data/fields_schema.template.csv data/fields_schema.csv
```

**Важно:** `fields_schema.csv` находится в `.gitignore`, чтобы ваши локальные изменения схемы (кастомные поля, опции) не конфликтовали с обновлениями из git. Редактируйте его под свои нужды!
...
- **employees.csv** - Основные записи сотрудников (40 полей):
  ...
  37. `notes` - Примечание
  38. `vacation_start_date` - Дата начала отпуска (YYYY-MM-DD)
  39. `vacation_end_date` - Дата окончания отпуска (YYYY-MM-DD)
  40. Остальные поля документов и служебных данных

- **fields_schema.csv** - **Мета-схема управления UI** (8 колонок, локальный файл в `.gitignore`):
  - `field_order` - Порядковый номер (1-40)
  ...
  - **Примечание:** Файл находится в `.gitignore` для независимости production от разработки. При первой установке создайте из `fields_schema.template.csv`
...
- При загрузке страницы система автоматически:
  - Меняет статус на значение "Отпуск" (второе значение из `employment_status` в `fields_schema.csv`) при наступлении даты начала
  ...
  - Очищает даты отпуска и восстанавливает первое значение из `employment_status` (обычно "Работает") после окончания

**Динамические статусы:**
- Статусы берутся из `fields_schema.csv` → `employment_status` → `field_options`
- При выходе из отпуска: первое значение (например: "Работает", "Активный")
- При уходе в отпуск: значение содержащее "отпуск" (например: "Отпуск", "В отпуске")
...
Поскольку `fields_schema.csv` находится в `.gitignore`, ваши локальные изменения схемы не будут конфликтовать с обновлениями:
...
- При запуске сервер автоматически сравнивает `fields_schema.csv` с `employees.csv`
- Если в схеме есть новые поля - они добавляются в `employees.csv` с пустыми значениями
- Все существующие данные сохраняются без изменений
- Миграция происходит прозрачно при первом старте после обновления

**Ручное обновление схемы (опционально):**
Если в новой версии добавлены важные поля в `fields_schema.template.csv`:

```bash
# Посмотрите изменения в template
git diff data/fields_schema.template.csv

# Вручную добавьте нужные поля в ваш локальный fields_schema.csv
