...
- [x] Запустити `cd server && npm test` — всі unit/integration тести проходять
- [x] Запустити `npm run test:e2e` — всі E2E тести проходять
- [x] Перевірити DocumentHistoryView — записи без employee_id відображаються коректно (без посилання на співробітника, або з позначкою "Загальний")
