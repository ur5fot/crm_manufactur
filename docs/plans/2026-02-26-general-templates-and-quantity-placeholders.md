...
- [ ] У POST `/api/templates/:id/generate` (`routes/templates.js:329-444`): замінити обов'язкову перевірку `employee_id` (~рядок 331-336) на умовну:
...
- [ ] Для загальних шаблонів: назва файлу `{TemplateName}_{timestamp}.docx` (без прізвища та employee_id)
- [ ] Для загальних шаблонів: `employee_id` в `generated_documents.csv` = порожній рядок
- [ ] Для загальних шаблонів: `data_snapshot` містить тільки quantity-ключі (без employee data)
- [ ] Написати тест: POST generate загального шаблону без employee_id → 200, є document_id
- [ ] Написати тест: POST generate звичайного шаблону без employee_id → 400
- [ ] Написати тест: POST generate загального шаблону з employee_id → теж працює (дані employee + quantities)
- [ ] Запустити `cd server && npm test`
...
- [ ] У `client/src/composables/useTemplatesManagement.js`:
...
- [ ] У `client/src/views/TemplatesView.vue` у діалозі створення (~після description, рядок 113):
...
- [ ] У таблиці шаблонів: відобразити значок/бейдж для загальних шаблонів (наприклад, поруч з назвою)
- [ ] Написати E2E тест `tests/e2e/general-templates.spec.js`:
...
- [ ] У `client/src/composables/useDocumentGeneration.js` (рядок ~9-10): після завантаження відфільтрувати:
  ```javascript
  templates.value = (data.templates || []).filter(t => t.is_general !== 'yes');
  ```
- [ ] Додати до E2E тесту `general-templates.spec.js`:
...
- [ ] Створити `client/src/views/GeneralTemplatesView.vue`:
...
- [ ] У `client/src/api.js`: додати метод:
  ```javascript
  ...
  ```
- [ ] У `client/src/views/DocumentsView.vue`:
