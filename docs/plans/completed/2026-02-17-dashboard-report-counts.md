...
- [x] Обидві кнопки показують кількість одразу при завантаженні дашборду (не треба клікати)
- [x] Якщо кількість = 0, дужки не показуються (логіка `> 0`)
- [x] При кліку на кнопку таблиця все ще відкривається і закривається
- [x] Дані таблиці відповідають кількості на кнопці
