...
**Subtasks:**
- [ ] Вручную протестировать concurrent PUT requests к одному employee
- [ ] Убедиться что второй запрос ждёт завершения первого
- [ ] Проверить что оба изменения корректно сохраняются последовательно
- [ ] Проверить что логи корректно записываются для обоих изменений
