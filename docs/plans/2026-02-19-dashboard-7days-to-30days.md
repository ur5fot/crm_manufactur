...
- [x] перевірити що `getDashboardEvents`, `getDocumentExpiryEvents`, `getBirthdayEvents` більше не використовують магічне число 7 для вікна (окрім де це явно 7 — як назви змінних)
- [x] запустити `cd server && npm test` — всі unit тести проходять
- [x] вручну перевірити дашборд у браузері: розділ відображається як "Найближчі 30 днів"
